...
function showBounceImage() {
  bounceImage.style.display = 'block';
  // After the animation finishes, hide the image
  setTimeout(() => {
    bounceImage.style.display = 'none';
  }, 1000);
}

// Add the event listener to the words that you want to trigger the animation
// (Assuming you have an element with the class "word" in your struggles.html)
const words = document.querySelectorAll('Scholar Struggles');
words.forEach(word => {
  word.addEventListener('click', showBounceImage);
});